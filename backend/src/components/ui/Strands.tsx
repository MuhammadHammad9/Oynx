  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({
        ...
        antialias: true,
        dpr: 1 // Cap device pixel ratio for major performance boost
      });
    } catch (error) {
      console.warn("Failed to initialize Strands WebGL renderer:", error);
      setRenderError(true);
      return;
    }
    ...
    return (
      <div
        ...
        style={{
          ...style,
          background:
            "radial-gradient(circle at 50% 35%, rgba(11, 189, 244, 0.18), transparent 38%), linear-gradient(180deg, rgba(255, 255, 255, 0.35), rgba(0, 0, 0, 0.18))"
        }}
      />
    );
  }
