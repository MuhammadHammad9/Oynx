...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!testGl) {
        console.warn("WebGL not supported, skipping LineWaves initialization");
        return;
      }
    } catch {
      console.warn("WebGL check failed, skipping LineWaves initialization");
      return;
    }
    ...
    try {
      ...
      console.warn("Failed to initialize WebGL for LineWaves:", err);
      setRenderError(true);
      return;
    }
  ...
  if (renderError) {
    return (
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(11, 189, 244, 0.16), transparent 45%), linear-gradient(135deg, rgba(14, 165, 233, 0.16), rgba(8, 47, 73, 0.12))"
        }}
      />
    );
  }
